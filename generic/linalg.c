...
int QRDecomposition(Tcl_Interp * interp, Tcl_Obj *matrix, Tcl_Obj **qr, Tcl_Obj **rdiag) {
	...
	*qr = Tcl_DuplicateObj(matrix);
	NumArrayUnshareBuffer(*qr);
	/* Now qr is a canonical copy of matrix */
	info = (*qr) -> internalRep.twoPtrValue.ptr2;
	char * bufptr;
	NumArrayGetBufferFromObj(NULL, (*qr), &bufptr);

	/* in canonical form we use this pointer */
	...
	*rdiag = Tcl_NewObj();
	...
	NumArraySharedBuffer *sharedbuf = NumArrayNewSharedBuffer(rdiaginfo->bufsize);
	NumArraySetInternalRep(*rdiag, sharedbuf, rdiaginfo);
	double *Rdiag = (double *)NumArrayGetPtrFromSharedBuffer(sharedbuf);
...
int QRsolve(Tcl_Interp *interp, Tcl_Obj *qr, Tcl_Obj *rdiag, Tcl_Obj *B) {
	...
	NumArrayGetBufferFromObj(NULL, Xobj, &bufptr);
	Xbuf = (double*) bufptr;

	NumArrayGetBufferFromObj(NULL, rdiag, &bufptr);
	Rdiag = (double *) bufptr;

	#define X(i, j)  (Xbuf[i*nx+j])
	#define QR(i, j) (QRbuf[i*n+j])
...
/*	return (new Matrix(X,n,nx).getMatrix(0,n-1,0,nx-1)); */
	...
	NumArrayInfo *Xinfo = Xobj -> internalRep.twoPtrValue.ptr2;
	Xinfo -> dims[0] = n;
	Tcl_InvalidateStringRep(Xobj);
    ...
    int objc, Tcl_Obj *const *objv) 
{
	...
	if (QRDecomposition(interp, objv[1], &qr, &rdiag) != TCL_OK) {
		return TCL_ERROR;
	}
    ...
    int objc, Tcl_Obj *const *objv) 
{
	...
	/* join the QR vector matrix and the diagonal elements
	 * int a list */
	if (QRsolve(interp, qr, rdiag, objv[2]) != TCL_OK) {
		return TCL_ERROR;
	}
